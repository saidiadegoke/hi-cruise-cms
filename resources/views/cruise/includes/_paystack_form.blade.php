<form method="POST" action="{{ route('flutterwave.pay') }}" accept-charset="UTF-8"  style="display: inline-block;" role="form">
            ...
            <input type="hidden" name="customizations" value="{{ json_encode(['title' => 'Hi-Impact Cruise', 'logo' => 'https://hi-impactcruise.com/public/assets/img/logo.png', 'description' => 'description',]) }}">
              ...
              <button class="btn btn-success btn-sm" type="submit" value="Pay Now!"> Pay Online!
              </button>
</form>

<!--<form method="POST" action="{{ route('pay') }}" accept-charset="UTF-8" style="display: inline-block;">
            <input type="hidden" name="email" value="{{ $reservation->email }}">
            <input type="hidden" name="orderID" value="{{ $reservation->id }}">
            <input type="hidden" name="amount" value="{{ doubleval($reservation->amount) * 100 }}"> 
            <input type="hidden" name="quantity" value="{{ $reservation->seats }}">
            <input type="hidden" name="metadata" value="{{ json_encode($array = ['reservation_id' => $reservation->id,]) }}" > 
